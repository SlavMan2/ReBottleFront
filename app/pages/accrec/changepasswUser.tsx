...
import { editPasswUser, logUserPhone, sendCodeUser } from '@/services/accounts';
...
export default function ChangePasswUserPage() {
  ...
  let handlePost = async (senha:string, code:any, routers:any,email:any) =>
  {
    ...
    let handler = await editPasswUser({code, senha, email,rnewpassw})
  ...
  return (
    ...
    <Button colort='black' colorback='#8DDFEB' text="voltar" functionToExecute={() => redirect('/pages/login')}></Button>
    </View>
  );
}
