...
import { logUserPhone, sendCodeUser } from '@/services/accounts';
...
export default function RequestCodeUser() {
  ...
  {
    let handler = await sendCodeUser({Ident,Mean})
    if (handler.response == true ) {
      return router.replace("/pages/accrec/changepasswUser");
    }
    else {
      console.log(handler)
      setErrMsg(handler)
  ...
  return (
    <View>
      ...
      <Text>{"\n"}</Text>
     <Button colort='black' colorback='#8DDFEB' text="Back" functionToExecute={() => redirect('/pages/login')}></Button>
    </View>
  );
}
